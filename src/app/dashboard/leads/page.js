...
export default function LeadsPage() {
  ...
  const [showSelectedLeadsCard, setShowSelectedLeadsCard] = useState(false);
  ...
  return (
    <div className="space-y-6 relative h-full">
          ...
          <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
            ...
            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg p-1.5 shadow-sm">
              ...
              <button
                ...
                className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-bold rounded shadow-sm transition cursor-pointer active:scale-95 border-0"
              >
                Go
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  if (!isNaN(fromVal) && !isNaN(toVal) && fromVal > 0 && toVal >= fromVal) {
                    ...
                    const slicedLeads = leads.slice(startIndex, endIndex).map(l => l._id);
                    setSelectedLeads(slicedLeads);
                    setShowSelectedLeadsCard(true);
                  } else if (selectedLeads.length > 0) {
                    setShowSelectedLeadsCard(true);
                  } else {
                    showToast('Please select a valid row range first.', 'error');
                  }
                }}
                className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold rounded shadow-sm transition cursor-pointer active:scale-95 border-0"
              >
                Show
              </button>
            </div>
      ...
      {/* Selected Leads Modal/Card */}
      {showSelectedLeadsCard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4 py-6 animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            {/* Header */}
            <div className="px-6 py-5 border-b border-slate-250 flex items-center justify-between bg-slate-50/50 rounded-t-2xl">
              <div className="flex items-center gap-2">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-100 text-indigo-755 text-[10px] font-black">
              ...
              <button 
                onClick={() => setShowSelectedLeadsCard(false)} 
                className="p-1 rounded-lg hover:bg-slate-200 text-slate-400 hover:text-slate-850 cursor-pointer border-0 bg-transparent"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            {/* List */}
            <div className="p-6 overflow-y-auto max-h-[350px] space-y-2.5 bg-white">
              {leads.filter(l => selectedLeads.includes(l._id)).length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-6 italic font-medium">No selected leads found on the current page.</p>
              ) : (
                leads
                  .filter(l => selectedLeads.includes(l._id))
                  .map((l, index) => (
                    <div 
                      key={l._id} 
                      className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl shadow-sm text-xs font-bold text-slate-755 hover:bg-indigo-50/20 hover:border-indigo-150 transition"
                    >
                      <div className="flex items-center gap-2.5">
                        <span className="text-[10px] text-slate-400 font-mono font-bold w-5">{leads.indexOf(l) + 1}</span>
                        <div className="flex flex-col gap-0.5">
                          <span className="text-slate-800 font-bold">{l.firstName} {l.lastName || ''}</span>
                          {l.company && (
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const newSelection = selectedLeads.filter(id => id !== l._id);
                          setSelectedLeads(newSelection);
                          if (newSelection.length === 0) {
                            setShowSelectedLeadsCard(false);
                          }
                        }}
                        className="text-slate-400 hover:text-rose-600 font-extrabold text-sm p-1 hover:bg-slate-200 rounded-lg transition border-0 bg-transparent cursor-pointer"
            ...
            {/* Footer */}
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50/50 rounded-b-2xl flex items-center justify-between gap-3">
              <button
                type="button"
                onClick={() => {
                  setSelectedLeads([]);
                ...
                className="px-4 py-2 border border-slate-250 hover:bg-slate-100 text-slate-600 text-xs font-bold rounded-lg transition cursor-pointer bg-white"
              >
                Clear All
              </button>
              <button
                type="button"
                onClick={() => setShowSelectedLeadsCard(false)}
                className="px-5 py-2.5 bg-indigo-650 hover:bg-indigo-600 text-white text-xs font-bold rounded-lg shadow-sm transition cursor-pointer"
              >
                Close
              </button>
            </div>
