              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!isNaN(fromVal) && !isNaN(toVal) && fromVal > 0 && toVal >= fromVal) {
                    ...
                    // Directly jump to the page containing fromVal
                    const targetPage = Math.ceil(fromVal / itemsPerPage);
                    if (targetPage >= 1 && targetPage <= totalPages) {
                      setCurrentPage(targetPage);
                      showToast(`Selected rows ${fromVal} to ${endIndex} (jumped to Page ${targetPage}).`);
                    } else {
                      showToast(`Selected rows ${fromVal} to ${endIndex}.`);
                    }
                  } else {
